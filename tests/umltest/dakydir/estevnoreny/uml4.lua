...
---@startuml{"fileName":"funkcia100.svg","fileType":"svg"}
-------------
--- x1-->y1
--- x1<---y1
--- @enduml
...
local function funkcia100(text,parser,extended)
   local result,errno
...
-- daky easy koment
local function add_moduleaaaaaaaaaa(tags,module_found,old_style)
...
--- c1<-b1:test response
--- @enduml
local function funkcia222222(tags,module_found,old_style)
      tags:add('name',module_found)
      tags:add('class','module')
      local item = F:new_item(tags,lineno())
      item.old_style = old_style
...
-- daky easy koment
local function add_bezkomentuuuuuuu(tags,module_found,old_style)
      tags:add('name',module_found)
      tags:add('class','module')
      local item = F:new_item(tags,lineno())
      item.old_style = old_style
...
---@startuml{"fileName":"funkcia300.svg","fileType":"svg"}
-------------
--- x1-->y1
--- x1<---y1
--- @enduml
...
--@ extended (any) nil if don't want to use extended explua grammar
--: (table,nil) Return a table with parsed infos
local function funkcia300(text,parser,extended)
   local result,errno
...
---@startuml{"fileName":"mimooo.svg","fileType":"svg"}
-------------
--- x1-->y1
--- x1<---y1
--- @enduml
...
--@ extended (any) nil if don't want to use extended explua grammar
--: (table,nil) Return a table with parsed infos
local function dalsiafunnnnnnnnnnnn(text,parser,extended)
   local result,errno
