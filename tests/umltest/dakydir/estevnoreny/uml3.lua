...
---@startuml{"fileName":"funkcia1.svg","fileType":"svg"}
-------------
--- x1-->y1
--- x1<---y1
--- @enduml
...
local function funkcia1(text,parser,extended)
   local result,errno
...
--- c1<-b1:test response
--- @enduml
local function funkcia2(tags,module_found,old_style)
      tags:add('name',module_found)
      tags:add('class','module')
      local item = F:new_item(tags,lineno())
      item.old_style = old_style
...
---@startuml{"fileName":"funkcia3.svg","fileType":"svg"}
-------------
--- x1-->y1
--- x1<---y1
--- @enduml
...
--@ extended (any) nil if don't want to use extended explua grammar
--: (table,nil) Return a table with parsed infos
local function funkcia3(text,parser,extended)
   local result,errno
...
---@startuml{"fileName":"dalsiafunnnn.svg","fileType":"svg"}
-------------
--- x1-->y1
--- x1<---y1
--- @enduml
...
--@ extended (any) nil if don't want to use extended explua grammar
--: (table,nil) Return a table with parsed infos
local function dalsiafunnnn(text,parser,extended)
   local result,errno
